...
export function renderChannelConfigForm(props: ChannelConfigFormProps) {
  ...
  if (!normalized) {
    return html`<div class="callout danger">${msg("Schema unavailable. Use Raw.")}</div>`;
  }
  const node = resolveSchemaNode(normalized, ["channels", props.channelId]);
  if (!node) {
    return html`<div class="callout danger">${msg("Channel config schema unavailable.")}</div>`;
  }
  ...
  return html`
    <div style="margin-top: 16px;">
      ${props.configSchemaLoading
        ? html`<div class="muted">${msg("Loading config schema…")}</div>`
      ...
      <div class="row" style="margin-top: 12px;">
        ...
        >
          ${props.configSaving ? msg("Saving…") : msg("Save")}
        </button>
        <button
          class="btn"
          ?disabled=${disabled}
