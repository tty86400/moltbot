  ...
  return html`
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${msg("Nodes")}</div>
          <div class="card-sub">${msg("Paired devices and live links.")}</div>
        </div>
      ...
      <div class="list" style="margin-top: 16px;">
        ${props.nodes.length === 0
          ? html`<div class="muted">${msg("No nodes found.")}</div>`
...
function renderDevices(props: NodesProps) {
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${msg("Devices")}</div>
          <div class="card-sub">${msg("Pairing requests + role tokens.")}</div>
        </div>
        <button class="btn" ?disabled=${props.devicesLoading} @click=${props.onDevicesRefresh}>
          ${props.devicesLoading ? msg("Loading…") : msg("Refresh")}
        </button>
      </div>
      ${props.devicesError
      ...
      <div class="list" style="margin-top: 16px;">
        ${pending.length > 0
          ? html`
              <div class="muted" style="margin-bottom: 8px;">${msg("Pending")}</div>
              ${pending.map((req) => renderPendingDevice(req, props))}
            `
          : nothing}
        ${paired.length > 0
          ? html`
              <div class="muted" style="margin-top: 12px; margin-bottom: 8px;">${msg("Paired")}</div>
              ${paired.map((device) => renderPairedDevice(device, props))}
            `
          : nothing}
        ${pending.length === 0 && paired.length === 0
          ? html`<div class="muted">${msg("No paired devices.")}</div>`
          : nothing}
      </div>
    </section>
  `;
...
function renderPendingDevice(req: PendingDevice, props: NodesProps) {
  ...
  const age = typeof req.ts === "number" ? formatAgo(req.ts) : msg("n/a");
  const role = req.role?.trim() ? msg(str`role: ${req.role}`) : msg("role: -");
  const repair = req.isRepair ? msg(str` · repair`) : "";
...
function renderPairedDevice(device: PairedDevice, props: NodesProps) {
  ...
  const roles = msg(str`roles: ${formatList(device.roles)}`);
  const scopes = msg(str`scopes: ${formatList(device.scopes)}`);
  ...
  return html`
    <div class="list-item">
      <div class="list-main">
        ...
        ${tokens.length === 0
          ? html`<div class="muted" style="margin-top: 6px;">${msg("Tokens: none")}</div>`
          : html`
              <div class="muted" style="margin-top: 10px;">${msg("Tokens")}</div>
...
function renderTokenRow(deviceId: string, token: DeviceTokenSummary, props: NodesProps) {
  const status = token.revokedAtMs ? msg("revoked") : msg("active");
  const scopes = msg(str`scopes: ${formatList(token.scopes)}`);
...
const SECURITY_OPTIONS: Array<{ value: ExecSecurity; label: string }> = [
  { value: "deny", label: msg("Deny") },
  { value: "allowlist", label: msg("Allowlist") },
  { value: "full", label: msg("Full") },
];

const ASK_OPTIONS: Array<{ value: ExecAsk; label: string }> = [
  { value: "off", label: msg("Off") },
  { value: "on-miss", label: msg("On miss") },
  { value: "always", label: msg("Always") },
];
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        <div>
          <div class="card-title">${msg("Exec node binding")}</div>
          <div class="card-sub">
            ${msg(str`Pin agents to a specific node when using `)} <span class="mono">exec host=node</span>.
          </div>
        </div>
        <button
          class="btn"
          ...
          @click=${state.onSave}
        >
          ${state.configSaving ? msg("Saving…") : msg("Save")}
        </button>
      </div>

      ${state.formMode === "raw"
        ? html`<div class="callout warn" style="margin-top: 12px;">
            ${msg(str`Switch the Config tab to `)} <strong>${msg("Form")}</strong> ${msg(str` mode to edit bindings here.`)}
          </div>`
        : nothing}

      ${!state.ready
        ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            <div class="muted">${msg("Load config to edit bindings.")}</div>
            <button class="btn" ?disabled=${state.configLoading} @click=${state.onLoadConfig}>
              ${state.configLoading ? msg("Loading…") : msg("Load config")}
            </button>
          </div>`
        : html`
            <div class="list" style="margin-top: 16px;">
              <div class="list-item">
                <div class="list-main">
                  <div class="list-title">${msg("Default binding")}</div>
                  <div class="list-sub">${msg("Used when agents do not override a node binding.")}</div>
                </div>
                <div class="list-meta">
                  <label class="field">
                    <span>${msg("Node")}</span>
                    ...
                    >
                      <option value="" ?selected=${defaultValue === ""}>${msg("Any node")}</option>
                      ${state.nodes.map(
                        (node) =>
                          html`<option
                            value=${node.id}
                  ...
                  ${!supportsBinding
                    ? html`<div class="muted">${msg("No nodes with system.run available.")}</div>`
                    : nothing}
                </div>
              </div>

              ${state.agents.length === 0
                ? html`<div class="muted">${msg("No agents found.")}</div>`
...
function renderExecApprovals(state: ExecApprovalsState) {
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        <div>
          <div class="card-title">${msg("Exec approvals")}</div>
          <div class="card-sub">
            ${msg(str`Allowlist and approval policy for `)} <span class="mono">exec host=gateway/node</span>.
          </div>
        </div>
        <button
          class="btn"
          ...
          @click=${state.onSave}
        >
          ${state.saving ? msg("Saving…") : msg("Save")}
        </button>
      </div>
      ...
      ${!ready
        ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            <div class="muted">${msg("Load exec approvals to edit allowlists.")}</div>
            <button class="btn" ?disabled=${state.loading || !targetReady} @click=${state.onLoad}>
              ${state.loading ? msg("Loading…") : msg("Load approvals")}
            </button>
          </div>`
        : html`
...
function renderExecApprovalsTarget(state: ExecApprovalsState) {
  ...
  return html`
    <div class="list" style="margin-top: 12px;">
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${msg("Target")}</div>
          <div class="list-sub">
            ${msg("Gateway edits local approvals; node edits the selected node.")}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            ...
            >
              <option value="gateway" ?selected=${state.target === "gateway"}>${msg("Gateway")}</option>
              <option value="node" ?selected=${state.target === "node"}>${msg("Node")}</option>
            </select>
          </label>
          ${state.target === "node"
            ? html`
                <label class="field">
                  ...
                  >
                    <option value="" ?selected=${nodeValue === ""}>${msg("Select node")}</option>
      ...
      ${state.target === "node" && !hasNodes
        ? html`<div class="muted">${msg("No nodes advertise exec approvals yet.")}</div>`
        : nothing}
    </div>
  `;
}
...
function renderExecApprovalsTabs(state: ExecApprovalsState) {
  return html`
    <div class="row" style="margin-top: 12px; gap: 8px; flex-wrap: wrap;">
      <span class="label">${msg("Scope")}</span>
...
function renderExecApprovalsPolicy(state: ExecApprovalsState) {
  ...
  return html`
    <div class="list" style="margin-top: 16px;">
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${msg("Security")}</div>
          <div class="list-sub">
            ${isDefaults
              ? msg("Default security mode.")
              : msg(str`Default: ${defaults.security}.`)}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            ...
            >
              ${!isDefaults
                ? html`<option value="__default__" ?selected=${securityValue === "__default__"}>
                    ${msg(str`Use default (${defaults.security})`)}
                  </option>`
                : nothing}
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${msg("Ask")}</div>
          <div class="list-sub">
            ${isDefaults ? msg("Default prompt policy.") : msg(str`Default: ${defaults.ask}.`)}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            ...
            >
              ${!isDefaults
                ? html`<option value="__default__" ?selected=${askValue === "__default__"}>
                    ${msg(str`Use default (${defaults.ask})`)}
                  </option>`
                : nothing}
              ${ASK_OPTIONS.map(
                (option) =>
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${msg("Ask fallback")}</div>
          <div class="list-sub">
            ${isDefaults
              ? msg("Applied when the UI prompt is unavailable.")
              : msg(str`Default: ${defaults.askFallback}.`)}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            ...
            >
              ${!isDefaults
                ? html`<option value="__default__" ?selected=${askFallbackValue === "__default__"}>
                    ${msg(str`Use default (${defaults.askFallback})`)}
                  </option>`
                : nothing}
              ${SECURITY_OPTIONS.map(
                (option) =>
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${msg("Auto-allow skill CLIs")}</div>
          <div class="list-sub">
            ${isDefaults
              ? msg("Allow skill executables listed by the Gateway.")
              : autoIsDefault
                ? msg(str`Using default (${defaults.autoAllowSkills ? "on" : "off"}).`)
                : msg(str`Override (${autoEffective ? "on" : "off"}).`)}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
...
function renderExecApprovalsAllowlist(state: ExecApprovalsState) {
  ...
  return html`
    <div class="row" style="margin-top: 18px; justify-content: space-between;">
      <div>
        <div class="card-title">${msg("Allowlist")}</div>
        <div class="card-sub">${msg("Case-insensitive glob patterns.")}</div>
      </div>
      <button
        class="btn btn--sm"
        ?disabled=${state.disabled}
    ...
    <div class="list" style="margin-top: 12px;">
      ${entries.length === 0
        ? html`<div class="muted">${msg("No allowlist entries yet.")}</div>`
...
function renderAllowlistEntry(
  ...
  const lastUsed = entry.lastUsedAt ? formatAgo(entry.lastUsedAt) : msg("never");
  ...
  return html`
    <div class="list-item">
      <div class="list-main">
        <div class="list-title">${entry.pattern?.trim() ? entry.pattern : msg("New pattern")}</div>
        <div class="list-sub">${msg(str`Last used: ${lastUsed}`)}</div>
  ...
  return html`
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="list-sub">
          ${agent.isDefault ? msg("default agent") : msg("agent")} ·
          ${bindingValue === "__default__"
            ? msg(str`uses default (${state.defaultBinding ?? "any"})`)
            : msg(str`override: ${agent.binding}`)}
        </div>
      </div>
      <div class="list-meta">
        <label class="field">
          <span>${msg("Binding")}</span>
          <select
            ?disabled=${state.disabled || !supportsBinding}
            @change=${(event: Event) => {
              const target = event.target as HTMLSelectElement;
...
function renderNode(node: Record<string, unknown>) {
  ...
  const title =
    (typeof node.displayName === "string" && node.displayName.trim()) ||
    (typeof node.nodeId === "string" ? node.nodeId : msg("unknown"));
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="chip-row" style="margin-top: 6px;">
          <span class="chip">${paired ? msg("paired") : msg("unpaired")}</span>
          <span class="chip ${connected ? "chip-ok" : "chip-warn"}">
            ${connected ? msg("connected") : msg("offline")}
          </span>
